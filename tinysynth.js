...
/*80*/[{w:"sine",t:0,f:1200,v:0.3,r:0.01,}],
    ...
    ready:function(){
      ...
      this.sustain=[]; this.notetab=[]; this.rhythm=[]; this.mute=[];
      this.settimers=[];
      this.recordings=[];	// references to arrays that are currently receiving recording of all MIDI events
	  this.noteNameArray=["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"];
	  this.noteNumberArray={"C":0,"C#":1,"Db":1,"D":2,"D#":3,"Eb":3,"E":4,"F":5,"F#":6,"Gb":6,"G":7,"G#":8,"Ab":8,"A":9,"A#":10,"Bb":10,"B":11};
	  this.midiCommands = [ "none","none","none","none","none","none","none","none",
	  ...
	  this.midiCommandNumbers = { "none": 0, "noteOff": 8, "noteOn": 9, "afterTouch": 10, "controller": 11, "patch": 12,
...
*/
    ...
    _note:function(t,ch,n,v,p){   
      ...
      if (this.useAudioTags()) {    // IE version-- we made a fake audio context with a timer, but it has no oscillator
        ...
        if (!this.rhythm[ch])    
          this.notetab.push({t:t,e:99999999,ch:ch,n:n,o:[],g:[],t2:t,v:0,r:[p[0].r],f:0,activeQuality:2,useAudioTags:true,audioTag:a,vel:v});
    ...
    setChMute:function(ch,m,t) {
      this.mute[ch]=m;
      if (this.useAudioTags()) return;
      this.chvol[ch].gain.setValueAtTime(this.vol[ch]*(this.mute[ch]?0:1)*this.ex[ch],this._tsConv(t));
    },
    ...
    allSoundOffAllDevices:function(ch) {  // unlike all sound off, this also sends messages to all current output devices.
      ...
      // external devices may have future notes scheduled with setTimeout.  These events must be canceled.
      for (var i = 0; i < this.settimers.length; i++) {
	if (this.settimers[i].ch==ch) {
	   try { clearTimeout(this.settimers[i].timer); } catch (ex1) { }
	   this.settimers.splice(i,1);
	   i--;
	}
      }
    },
    ...
    resetAllControllers:function(ch){
      ...
      if(this.chvol[ch]){
        this.chvol[ch].gain.value=this.vol[ch]*this.ex[ch]*(this.mute[ch]?0:1);
    ...
    send:function(msg,t,norecord){   /* DW new: Public version of send that sends the midi message to whichever output sources are currently configured! */
      var midi = this;
	  // norecord is optional, if true, recording does not happen
	  norecord = !!norecord;
      // record the message on any current recording streams
	  if (!norecord) { this.recordMsg(msg,t); }
	  // check for program overrides on the current channel.
    ...
    sendExternal:function(device,msg,t) {  // Sends a MIDI message to a particular external port, which can be either a MIDI output object, OR, an ID string to locate the object.
      if (msg && Array.isArray(msg) && msg.length > 2 && (msg[0] & 0xF0)==0xC0) { msg = [ msg[0], msg[1] ]; }  // program changes have to be two bytes to be accepted by external devices
    ...
    _sendExternalOnDelay:function(msg,device,t) {
      ...
      var normalClear = function() {};
      var timerId = setTimeout(function() { 
	try { 
		device.send(msg); 
		normalClear(); 
	} catch(e) { } }, (t-this.actx.currentTime)*1000);
      this.settimers.push({ ch: (msg[0] & 0x0F), timer: timerId});  // save the timer so it can be cancelled later
      var clearSynth = this;
      normalClear = function() {
        try {
	  for (var i = 0; i < clearSynth.settimers.length; i++) {
	     if (clearSynth.settimers[i].timer == timerId) { clearSynth.settimers.splice(i,1); break; }
	  }
	} catch (ncex1) { }
      };
    },
...
//            console.log("UseForRhythmPart("+ch+")="+msg[8]);
    ...
    record: function(destArray, fixedLength) {
		...
		for (var i = 0; i < 16; i++) {	// all recordings include set-program (instrument) for all channels, and then when you stop recording, it removes the ones that weren't used.
			if (i==9) continue;
			destArray.push({"type":"midi","msg":[0xC0+i,this.pg[i]],"t":pgmt,"initsetprog": true, "initsetch":i });
		}
		for (var i = 0; i < this.notetab.length; i++) {	// record all the notes that are already down
			destArray.push({"type":"midi","t":pgmt,"msg":[0x90+(0x0F & this.notetab[i].ch), (0x7F & this.notetab[i].n), (0x7F & this.notetab[i].vel)]});
		}
    ...
    stopRecord: function(destArray) {
		...
		// find out which channels were used, and remove the initial set-program MIDI's that don't apply
		var channelsUsed = this.channelsInRecording(destArray);
		for (var i = 0; i < destArray.length; i++) {
			if (destArray[i].initsetprog) {
		...
		for (var i = 0; i < this.recordings.length; i++) {
			if (this.recordings[i]===destArray) { this.recordings.splice(i,1); i--; }
		}
    },
	cancelPrerecordedNotes() {
		// when playing back on a loop, you may send notes in advance, and then have to cancel them.
		// in that case you have to also un-record the upcoming notes from all recordings.
		var nowTime = this.currentTime();
		for (var i = 0; i < this.recordings.length; i++) {
			var thisRecording = this.recordings[i];
			for (var j = 0; j < thisRecording.length; j++) {
				if (thisRecording[j].t >= nowTime) { thisRecording.splice(j,1); j--; }
			}
		}
	},
	sortRecordingByTime(r) {
		// sorts a recording by the time field
		// it might not already be like that if we are looping an existing playback and also playing  
		r.sort(function (a,b) {
			return a.t-b.t;
		});
		return r;
	},
	channelsInRecording: function(r) {
		// which channels are used in the provided recording?
		var outArray = [];
		for (var i = 0; i < r.length; i++) {
			if (r[i].initsetprog) continue;
			if (r[i].type=="midi") {
				var midiCmd = (r[i].msg[0] & 0xF0);
				if (midiCmd>=0x80 && midiCmd<=0xE0) {
					var midiCh = (r[i].msg[0] & 0x0F);
					if (!outArray.includes(midiCh)) outArray.push(midiCh);
				}
			}
		}
		return outArray;
	},
	notesInRecording: function(r) {
		// which notes are used in the provided recording?
		var outArray = [];
		for (var i = 0; i < r.length; i++) {
			if (r[i].initsetprog) continue;
			if (r[i].type=="midi") {
				var midiCmd = (r[i].msg[0] & 0xF0);
				if (midiCmd>=0x80 && midiCmd<=0x90) {
					var midiNote = r[i].msg[1];
					if (!outArray.includes(midiNote)) outArray.push(midiNote);
				}
			}
		}
		outArray.sort();
	...
	firstNoteTime: function(r) {
		// time of first note-on in the recording; returns NaN if no notes
		for (var i = 0; i < r.length; i++) {
			if (r[i].initsetprog) continue;
			if (r[i].type=="midi") {
				var midiCmd = (r[i].msg[0] & 0xF0);
				if (midiCmd==0x90) return r[i].t;
			}
		}
		return NaN;
	},
	maxTime: function(r) {
		// return the maximum t value from r, usually, the time that the stop/loop button was pressed
		var maxT = 0;
		for (var i = 0; i < r.length; i++) {
			if (r[i].t > maxT) maxT = r[i].t;
		}
		return maxT;
	},
	...
	recordedNotesAfterTime(r,t) {
		// creates a clone of only the messages in r that are after t
		var r2 = [];
		for (var i = 0; i < r.length; i++) {
			if (r[i].t>t) r2.push(JSON.parse(JSON.stringify(r[i])));
		}
		return r2;
	},
	recordMsg: function(inmsg,t) {
		// record a given midi message to all the currently recording arrays
		// automatically done whenever you send a midi message using the public send interface
		if (!t) t = this.currentTime();  // t is optional, and if not provided, we use the current time
	...
	nextMessageAfter(r,t) {
		// returns the index in r for the next message after time t.
		for (var i = 0; i < r.length; i++) {
			if (r[i].t>=t) return i;
		}
		return r.length;
	...
	sendRecordedNotes: function (r, doNotRecordPlayback, startIndex) {
		...
		return {
			...
			stop: function() {
				...
				if (bufferFillTimer!=null) { clearTimeout(bufferFillTimer); bufferFillTimer = null; }
			}
		}
	},
	combineRecordings(r1,r2,mode) {
	...
	playback: function(r, t, doNotRecordPlayback, parentPlayControl, onlyAfter, startAccKeys) {
		...
		// returns a special playback control entity that can be used to pause, loop, change accompaniment settings for channels, play accompaniment notes, etc.
		// generally you don't pass in a value for parentPlayControl, which is only used internally for looping recursion
		// same deal with onlyAfter: this is normally not passed by callers, but is used recursively, if non-null it indicates to start
		// at that index in r.
		var mysynth = this;
		if (!t) t = this.currentTime();
		doNotRecordPlayback = !!doNotRecordPlayback;
		var sourceR = r; var startIndex = 0;
		var baseTime = t;
		if (onlyAfter !== null && onlyAfter != undefined) {
		...
		for (var i = 0; i < 16; i++) {
			accompSettingsArray.push(JSON.parse(JSON.stringify({"type":"none","key":0,"notes":[]})));  // accompaniment settings for each midi channel
		}
		var playControl = parentPlayControl;
		if (!playControl) {
			playControl = {		// this structure is returned to the caller and helps them manage the playback
				"notes": r,
				"loop": false,
				"accomp": accompSettingsArray,
				"paused": false,
				"finished": false,  // only true if all loops have finished
				"onEnd": function(){},		// you can set this if you want your own on-stop function
				"endTimer": null,
				"getLastPlayedIndex": function() {
					return playControl.currentSendControl.getLastPlayedIndex();
				},
				...
				"stop": function() {
					...
					if (playControl.endTimer !== null) clearTimeout(playControl.endTimer);
					playControl.endTimer = null;
				},
				"togglePause": function() {
					if (playControl.paused) {
						// stop pause
						playControl.paused = false;
						try {	// when we stop the pause, we turn back on the notes that we turned off when we paused.
							if (playControl.notesOnDuringPause) { 
								for (var i = 0; i < playControl.notesOnDuringPause; i++) {
									mysynth.send(playControl.notesOnDuringPause[i]);
								}
							}
						} catch (ex1) { }
						mysynth.playback(r,mysynth.currentTime(),doNotRecordPlayback,playControl,playControl.continueAfter);
					} else {
						// start pause
						playControl.paused = true;
						playControl.currentSendControl.stop();
						if (playControl.endTimer !== null) { clearTimeout(playControl.endTimer); playControl.endTimer = null; }
						playControl.continueAfter = 1 + playControl.currentSendControl.getLastPlayedIndex();
						playControl.notesOnDuringPause = playControl.currentSendControl.getCurrentNotesOn();
					}
				}
			};
		}
	...
	noteName: function(n) {
		if (n<0) n = 0;
		return this.noteNameArray[n%12];
	},
	noteOctave: function(n) {
		if (n<0) n = 0;
		return Math.floor(n/12)-1;
	},
	noteNumber: function(n,o) {
		if (this.noteNumberArray[n]===undefined) return NaN;
		return this.noteNumberArray[n]+((o+1)*12);
	},
	recordingToEnglish(r) {
		// converts a recording (from record/stopRecord) into readable text that can be edited in a text editor
		var outstr = "";
		for (var i = 0; i < r.length; i++) {
			switch (r[i].type) {
				case "timestarted":
					outstr += "starttime(" + r[i].t + ");\r\n";
					break;
				case "timestopped":
					outstr += "stoptime(" + r[i].t + ");\r\n";
					break;
				case "midi":
					try {
						if (r[i].msg) {
							var midiCmd = ((r[i].msg[0])&0xF0)>>4;
							var midiCh = (r[i].msg[0])&0x0F;
							var midiNoteNum = r[i].msg[1];
							var midiVel = 0;
							if (r[i].msg.length>2) midiVel = r[i].msg[2] & 0x7F;
							var midiCmdName = this.midiCommands[midiCmd];
							if (midiCmd==8 || midiCmd==9) {
								outstr += midiCmdName + "('" + this.noteName(midiNoteNum) + "'," + this.noteOctave(midiNoteNum) + "," + midiCh + "," + midiVel + "," + r[i].t + ");\r\n";
							} else {
								if (r[i].msg.length>2) {
									outstr += "midi('" + midiCmdName + "'," + midiCh + "," + midiNoteNum + "," + midiVel + "," + r[i].t + ");\r\n";
								} else {
									outstr += "midi('" + midiCmdName + "'," + midiCh + "," + midiNoteNum + "," + r[i].t + ");\r\n";
								}
							}
						}
					} catch (midiex1) { }
